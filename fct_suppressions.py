...
def suppression_type(coll) :
    while True :
        type_sup_input = input(colorama.Fore.YELLOW + "Entrez le type à supprimer (livre ou article) : " + colorama.Style.RESET_ALL)
        if type_sup_input.lower() == "livre" :
            type_sup = "Book"
        elif type_sup_input.lower() == "article" :
            type_sup = "Article"
        else :
            print("Veuillez reformuler votre demande - en inscrivant livre ou article")
